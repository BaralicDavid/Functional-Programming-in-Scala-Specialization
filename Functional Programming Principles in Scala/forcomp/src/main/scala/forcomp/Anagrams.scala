...
object Anagrams extends AnagramsInterface:
  ...
  def wordOccurrences(w: Word): Occurrences =
   w.toLowerCase.groupBy(identity)
     .map { case (char, occurrences) => (char, occurrences.length) }.toList.sorted
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    occurrences match
      ...
      case (char, count) :: rest =>
        for
          n <- (0 to count).toList
          restComb <- combinations(rest)
        yield if n == 0 then restComb else (char, n) :: restComb
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    y.foldLeft(x.toMap) { case (map, (char, count)) =>
      ...
      if newCount > 0 then map.updated(char, newCount) else map - char
    }.toList.sorted
